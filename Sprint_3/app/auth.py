    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                ...
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'UPDATE user SET password = ? AND salt = ? WHERE id = ?', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
        ...
        return render_template('auth/forgot.html') #### revisar
    except:
        return render_template('auth/forgot.html')
...
def send_email(credentials, receiver, subject, message):
    ...
    # Send Email
    smtp = smtplib.SMTP("smtp-mail.office365.com", port=587)
    smtp.starttls()
